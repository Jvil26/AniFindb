...
router.post('/register', async (req, res) => {
   try {
      ...
      if (error) {
         const parsedError = error.details[0].message.replace(/["]/g, "").split(" ");
         parsedError[0] = parsedError[0].charAt(0).toUpperCase() + parsedError[0].slice(1);
...
router.post('/login', async (req, res) => {
   try {
      ...
      if (!validPass) return res.status(400).send({ message: 'Password is invalid'});
